...
export function MatrizPresupuesto({
  ...
  return (
    <div data-testid="matriz-presupuesto" className="space-y-6 p-4">
        ...
        <div className="mt-2 overflow-x-auto">
          <table className="min-w-full divide-y divide-white/10 text-sm">
            <thead className="bg-zinc-800">
              <tr>
                <th className="px-3 py-2 text-left text-xs font-medium uppercase text-slate-400">
                  Categoría
                </th>
                <th className="px-3 py-2 text-right text-xs font-medium uppercase text-slate-400">
                  Fijo
                </th>
                <th className="px-3 py-2 text-right text-xs font-medium uppercase text-slate-400">
                  Variable
            ...
            <tbody className="divide-y divide-white/10 bg-zinc-900">
              {matriz.ingresos.map((row) => (
                <tr key={row.categoria}>
                  <td className="px-3 py-2 text-slate-100">{row.categoria}</td>
      ...
      >
        <h2 className="text-base font-semibold text-white">Totales</h2>
        <dl className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
          <div>
            <dt className="text-xs uppercase text-slate-400">Total ingresos</dt>
            <dd className="text-base font-mono font-medium text-green-700">
              {formatCentavosConDecimales(matriz.totalIngresos.toNumber())}
            </dd>
          </div>
          <div>
            <dt className="text-xs uppercase text-slate-400">Total gastos</dt>
            <dd className="text-base font-mono font-medium text-red-700">
              {formatCentavosConDecimales(matriz.totalGastos.toNumber())}
            </dd>
          </div>
          <div>
            <dt className="text-xs uppercase text-slate-400">Flujo de caja libre</dt>
            <dd
