    ...
    <div data-testid="lista-transacciones" className="overflow-x-auto">
      <table className="min-w-full divide-y divide-slate-200 dark:divide-white/10">
        <thead className="bg-slate-100 dark:bg-zinc-800">
          <tr>
            <th className="px-4 py-2 text-left text-xs font-medium uppercase text-slate-500 dark:text-slate-400">
              Concepto
            </th>
            <th className="px-4 py-2 text-left text-xs font-medium uppercase text-slate-500 dark:text-slate-400">
              Tipo
            ...
            <th className="px-4 py-2 text-right text-xs font-medium uppercase text-slate-500 dark:text-slate-400">
              Acciones
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-200 dark:divide-white/10 bg-white dark:bg-zinc-900">
          {transacciones.map((t) => (
            <tr key={t.id} data-testid="fila-transaccion">
              <td className="px-4 py-2 text-sm text-slate-900 dark:text-slate-100">{t.concepto}</td>
              ...
              <td className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300">{t.frecuencia}</td>
              <td className="px-4 py-2 text-sm font-mono text-slate-900 dark:text-slate-100">
                {formatCentavos(t.valor_centavos)}
              </td>
              <td className="px-4 py-2 text-sm text-slate-600 dark:text-slate-300">{t.categoria_nombre}</td>
              <td className="px-4 py-2 text-right">
                <button
                  type="button"
                  data-testid={`editar-${t.id}`}
                  onClick={() => onEditar(t.id)}
                  className="mr-3 text-xs text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-100"
                >
                  Editar
                </button>
                <button
