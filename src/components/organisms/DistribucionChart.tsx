...
export function DistribucionChart({ distribucion, titulo }: DistribucionChartProps): JSX.Element {
  if (distribucion.length === 0) {
    return (
      <div data-testid="distribucion-chart" className="p-4">
        <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300">{titulo}</h3>
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">No hay datos para mostrar.</p>
      </div>
    )
  }
  ...
  return (
    <div
      ...
      className="rounded-md border border-slate-200 dark:border-white/10 bg-white dark:bg-zinc-900 p-4"
    >
      <h3 className="text-sm font-medium text-slate-700 dark:text-slate-300">{titulo}</h3>
